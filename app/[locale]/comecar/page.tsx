...
import { getPlans as getPlanContent } from "@/lib/content/plans";
import { cn } from "@/lib/utils";

const sectorIcons: Record<Sector, LucideIcon> = {
  [Sector.RESTAURANT]: Utensils,
  [Sector.HAIR_SALON]: Scissors,
  [Sector.HOTEL]: Hotel,
  [Sector.GYM]: Dumbbell,
  [Sector.LOCAL_SERVICES]: Handshake,
  [Sector.SMALL_SHOP]: Store,
  [Sector.CLINIC]: Stethoscope,
  [Sector.OTHER]: Sparkles,
};
...
export default function ComecarPage({
  ...
  const locale: Locale = isLocale(params.locale)
    ? params.locale
    : DEFAULT_LOCALE;
  const t = getOnboardingCopy(locale);
  const trialBadge = getDictionary(locale).trialBadge;
  const planContent = useMemo(() => getPlanContent(locale), [locale]);
  ...
  // Numeric prices come from the live plans API; localized name/features come
  // from the per-locale content registry (lib/content/plans.ts).
  const recommendedPlan = plans?.find(
    (p) => p.code === recommendation.planCode
  );
  const recommendedContent = planContent.find(
    (p) => p.code === recommendation.planCode
  );
  ...
  async function submit() {
    ...
    try {
      ...
      toast({
        ...
        description: t.toastCreatedDesc,
      });
      ...
      toast({
        ...
        description: err instanceof Error ? err.message : t.toastErrorFallback,
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
  ...
  const stepMeta = [
    t.steps.business,
    t.steps.needs,
    { ...t.steps.plan, subtitle: t.reasons[recommendation.reasonKey] },
    t.steps.account,
  ][step - 1];
        ...
        <div className="flex-1">
          {step === 1 && (
            <div className="space-y-8">
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                {sectorOrder.map((value) => {
                  const selected = sector === value;
                  const Icon = sectorIcons[value];
                  return (
                    <button
                      key={value}
                      type="button"
          ...
          {step === 2 && (
            <div className="space-y-2">
              <Toggle
                label={t.toggles.needsBookings}
                checked={needsBookings}
                onChange={setNeedsBookings}
              />
              <Toggle
                label={t.toggles.multipleStaff}
                checked={multipleStaff}
                onChange={setMultipleStaff}
              />
              <Toggle
                label={t.toggles.weeklySchedule}
                checked={weeklySchedule}
                onChange={setWeeklySchedule}
              />
              <Toggle
                label={t.toggles.multipleLocationsOrRooms}
                checked={multipleLocationsOrRooms}
                onChange={setMultipleLocationsOrRooms}
              />
              <Toggle
                label={t.toggles.teamUsers}
                checked={teamUsers}
                onChange={setTeamUsers}
              />
              <Toggle
                label={t.toggles.payments}
                checked={payments}
                onChange={setPayments}
              />
              <Toggle
                label={t.toggles.externalIntegrations}
              ...
              {recommendedPlan ? (
                <div className="rounded-xl border border-line/40 bg-surface-low p-5">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="text-headline-sm font-bold text-brand">
                        {recommendedContent?.name ?? recommendedPlan.name}
                      </div>
                    </div>
                  ...
                  <ul className="mt-4 space-y-2">
                    {(
                      recommendedContent?.features ?? recommendedPlan.features
                    ).map((f) => (
                ...
                <p className="text-body-md text-ink-muted">{t.loadingPlan}</p>
              )}
